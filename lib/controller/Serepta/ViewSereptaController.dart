...
class ViewSereptaController extends GetxController{
 
  SqlDb sqlDb = SqlDb();
  List<SereptaModel> data = [];

  StatusRequest statusRequest = StatusRequest.none;

  SereptaModel? sereptaModel;
  SereptaData sereptaData = SereptaData(Get.find());
...
getDataFromOnlineToOffLine() async {
  ...
  if (connectivityResult == ConnectivityResult.none) {
    ...
    List<Map<String, dynamic>> offlineData = await sqlDb.getAllData('serepta');
    if (offlineData.isNotEmpty) {
       data.clear();
      data.addAll(offlineData.map((e) => SereptaModel.fromJson(e)));
  ...
  var response = await sereptaData.getData();
  statusRequest = handlingData(response);

  if (StatusRequest.success == statusRequest) {
if (response['status'] == "success") {
  List datalist = response['data'];
  List<SereptaModel> sereptaList = datalist.map((e) => SereptaModel.fromJson(e as Map<String, dynamic>)).toList();
  data.addAll(sereptaList);

  // Iterate over the data and insert into the SQLite database
  await sqlDb.syncData('serepta', sereptaList.map((e) => e.toJson()).toList(), ConflictAlgorithm.replace);
} else {
